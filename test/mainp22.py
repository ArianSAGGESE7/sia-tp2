...
def genetic_algorithm(ref_img, opciones=opciones):
    ...
    for gen in range(num_generaciones):
        ...
        for ind in poblacion:
            ...
            ta = time()
            ind.costo = calcular_costo(img, ref_img,gen)
            
            prom_gen.append(ind.costo) 
            tb = time()
            t_eval += tb - ta

        # ordeno en funcion de costo
        t1 = time()
        fitness_prom_gen.append(np.mean(prom_gen))
        prom_gen = []
        ...
        p1, p2 = np.random.choice(posibles_padres, 2)
        hijo = cruzar(p1, p2, metodo_de_cruza, **kwargs_cruza)
        hijo = mutar(hijo, 0.9, 0.9)
        nueva_poblacion[num_seleccion_elite] = hijo;
        poblacion = nueva_poblacion
        
        if  nueva_poblacion[num_seleccion_elite+1].costo < poblacion[0].costo:
            p1, p2 = np.random.choice(posibles_padres, 2)
            hijo = cruzar(p1, p2, metodo_de_cruza, **kwargs_cruza)
            hijo = mutar(hijo, 0.9, 0.9)
            poblacion[1] = hijo;
                ...
                # poblacion[num_seleccion_elite] = hijo
        ## No usar todos hijos sino que completar con un parametro alpha y después poner
        ## los mejores padres de manera tal de tner siempre el mismo len de poblacion
